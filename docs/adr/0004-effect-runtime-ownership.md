...
7. Google translation runs as one target-specific structured document per Workpool job. Live-caption targets run in their own Workpool with parallelism ten; explicit retries run in a separate Workpool with parallelism two, so retries never occupy live-caption slots. Each target has a 20-second timeout and at most three total transient attempts inside the action with exponential backoff, jitter, and `Retry-After` support. An explicit retry re-translates only the failed targets of the Accepted commit. Scribe token acquisition gets at most two short transient retries within a ten-second total bound. Every typed provider failure, including exhausted transient attempts, is a validated failed completion; a thrown action becomes a failed target through the Workpool completion without a Workpool retry.
